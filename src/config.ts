export const siteConfig = {
  ...
  projects: [
    ...
    },
    {
      ...
      description:
            "Oversynchronisation of GPUs : This research work is about investigating 'over-synchronization' in CUDA programs—a performance bottleneck where developers use wider synchronization scopes than necessary for correctness. The study outlines the interplay between the PTX memory consistency model and GPU hardware behaviors (like L1/L1 cache bypassing). It investigates how redundant device-scoped fences and atomic RMW instructions impact throughput, leading to the development of a dynamic analysis tool that improves GPU library performance by up to 55%",
    ...
    },
    {
      ...
      description:
            "This project involved developing features for the server system - the Secure Debug Unlock and Microcode Patching infrastructure, on AMD Daytona and Ethanol/EthanolX boards. The work centered on designing a secure HLD/LLD interface, and developing the code for the same, that allows authenticated access to sensitive security processor registers via the Hardware Debug Tool (HDT), preventing unauthorized hardware-level intrusion. It further integrated microcode complex patching and the APEX architecture, which utilizes BMC controllers for remote diagnostics, power state manipulation, and automated firmware testing to ensure platform integrity in high-scale data center environments",
      link: " ",
