export const siteConfig = {
  ...
  aboutMe:
    "System Software/Firmware developer with 7+ years of hands-on experience in multimedia embedded systems, real-time audio DSP, multimedia frameworks, and fault-tolerant embedded systems, bootloader design, and real-time operating systems. Having expertise in BIOS/UEFI development, Coreboot porting, signal processing algorithms and system-level optimization across automotive, server, IoT, and wearable domains. Passionate about hardware-software co-design, low-power embedded systems, and bringing production-grade firmware to resource-constrained environments. Collaborated with Qualcomm, AMD, Harman and demonstrated consistent ability to reduce boot times, optimize power consumption, implement full-duplex and ahalf-duplex audio communication systems from scratch for hospital infrastructure and solve complex system-level challenges in high-performance embedded systems.",
  ...
  projects: [
    ...
    },
    {
      ...
      description:
            "The project outlines the implementation of critical system-level features—Subsystem Reset (SSR) and Hands-Free Profile (HFP).The project investigates methods for fault-tolerant computing within the audio processor. It outlines the development of an SSR framework designed to detect DSP-side crashes and orchestrate an independent recovery of the audio subsystem. And the work of HFP feature development investigates the end-to-end voice path within the Signal Processing Framework (SPF). It outlines the development of Hands-Free Profile (HFP) support in HAL layer, utilizing the Common Audio Processing Interface (CAPI) to manage complex voice topologies. The project investigates the synchronization of host-side Bluetooth protocols with embedded DSP firmware to enable low-latency, high-fidelity concurrent PCM transcoding for wearable platforms",
      link: " ",
